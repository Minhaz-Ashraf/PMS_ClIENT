...
const CancelledPolicyData = () => {
  ...
  const {data, totalPoliciesCount, totalPagesCount} = useSelector((state) => state.admin.fetchCancelledData);
      ...
      <div className="font-head pt-9">
        ...
        <div className="flex flex-row justify-between md:mx-36 mx-6 md:ml-[18%] sm:ml-[33%] w-[240%]  bg-secondary  md:w-[75%] py-2 px-6 sm:w-[120%] font-semibold text-center text-[16px] items-center font-DMsans mb-6">
          ...
          {filteredPolicies.map((item, index) => (
            <AgentTableData key={index} item={item} index={index} />
          ))}
          </div>
        )}
...
const AgentTableData = ({ item, index }) => {
  const pdfRef = useRef();
  ...
  return (
    ...
    <div className="flex text-[14px] flex-row justify-between items-start w-full my-1">
        ...
        <Link to = "/policy"
          ...
          <span onClick={handleDownloadClick} className="bg-primary text-white py-1 px-3 cursor-pointer rounded-md">
            Download
          </span>
        </p>
