      ...
      <div className="overflow-x-scroll w-full md:w-full md:overflow-hidden ">
        ...
        <div>
          {loading ? (
            ...
            pendingPolicy?.data
              ?.filter((policy) => policy.policyStatus === "yetToApproved" || policy?.isCancelReq === "reqCancel")
              .map((item, index) => (
                <ApprovalCard
                  key={item._id}
                  item={item}
    ...
    <>
      <ul className="text-[15px] flex flex-row justify-around items-start mx-6 sm:mx-6 md:mx-10 md:ml-72 sm:ml-72 gap-2 rounded-lg mt-8 text-black font-normal w-[180%] md:w-[80%] sm:w-[100%]">
        ...
        <li className="w-[36%] px-3 text-start mb-3 py-3 rounded-lg bg-secondary  shadow">
          ...
          {agentData?.agentName}   {item?.isCancelReq === "reqCancel" ? "Sent a request to cancel the Policy of" :" Sent a request to approve the policy of" }
         {" "} {item?.customerName} {" "} Id: {item?.isCancelReq === "reqCancel" ? item?.policyId : ""}
          <Link
            to="/policy"
            state={{ id: item?._id }}
