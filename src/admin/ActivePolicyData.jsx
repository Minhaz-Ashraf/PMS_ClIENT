...
const ActivePolicyData = () => {
  ...
  }, [dispatch, selectedManufacturer, currentPage]);
  useEffect(() => {
    const filtered = (Array.isArray(policy) ? policy : []).filter(
      (item) =>
        !item.isDisabled &&
        item.vehicleManufacturer === selectedManufacturer &&
        (item.policyId?.includes(searchTerm) ||
          item.vehicleRegNumber?.includes(searchTerm) ||
          item.engineNumber?.includes(searchTerm))
    );
    setFilteredPolicies(filtered);
  }, [policy, selectedManufacturer, searchTerm]);
...
const PolicyDataTable = ({ item, index, handleCancelPolicy }) => {
  const pdfRef = useRef();
