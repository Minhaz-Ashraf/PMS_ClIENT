...
const AdminCards = ({ name, agentId, link, handleDelete, id, editLink }) => {
  const [isPopUpOpen, setIsPopUpOpen] = useState(false);
  const openPopUp = useCallback(() => setIsPopUpOpen(true), []);
  const closePopUp = useCallback(() => setIsPopUpOpen(false), []);
  return (
    <>
      <div className="bg-secondary shadow flex flex-col items-center w-60 rounded-md pt-5 mt-6">
        <p className="font-medium  ">{name} </p>
        <p className="font-medium pt-1">{agentId}</p>
        <Link
          to={link}
          state={{ agentId: id }}
          className="text-white  bg-primary mt-2 px-16 py-2 cursor-pointer rounded-md"
        >
          View More
        </Link>
